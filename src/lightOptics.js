...
export function traceLightRays(specs, worldBounds) {
  ...
  const obstacles = specs.filter((s) => s.type !== "lightSource" && s.type !== "lens");
  ...
  for (const src of sources) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      rays.push(traceOneRay(origin, dir0, obstacles, lenses, worldBounds));
    }
  }
  return rays;
}

function traceOneRay(origin, dir, obstacles, lenses, bounds) {
  const points = [origin];
  let o = origin, d = dir, medium = AIR_N;
  let insideId = null; // spec id of the shape we're currently inside, if any

  for (let bounce = 0; bounce < MAX_BOUNCES; bounce++) {
    let nearest = null, nearestSpec = null, nearestIsLens = false, nearestOpaque = false;

    for (const spec of obstacles) {
      const n2 = refractiveIndexOf(spec);
      const hit = shapeHit(o, d, spec);
      if (hit && (!nearest || hit.t < nearest.t)) { nearest = hit; nearestSpec = spec; nearestIsLens = false; nearestOpaque = !n2; }
    }
    for (const lens of lenses) {
      const hit = bendThroughLens(o, d, lens);
      if (hit && (!nearest || hit.t < nearest.t)) { nearest = hit; nearestSpec = lens; nearestIsLens = true; nearestOpaque = false; }
    }
