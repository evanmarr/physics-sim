...
export class PhysicsSim {
  ...
  _createBody(spec, forceDynamic = false, noCollideGroup = null, isPivotBearing = false) {
    ...
    switch (spec.type) {
      ...
      case "customPolygon": {
        ...
        // saved vertex list, not a stand-in circle/rectangle. Only simple
        // (non-self-intersecting) shapes are ever saved (see
        // src/customItems.js's validation); concave ones are decomposed
        // into convex parts automatically by poly-decomp, wired in via
        // Matter.Common.setDecomp (also in src/customItems.js) — so what
        // you see in the editor is exactly what collides here either way.
  ...
  enableGrabTool(shape = "ball") {
    ...
    const bodyOpts = {
      ...
      frictionAir: 0.02,
      restitution: 0.3,
      // A real pointer shouldn't tumble as it bumps into things — infinite
      // inertia makes it immune to the angular impulses a normal collision
      // would otherwise impart, so it stays upright and only ever
      // translates, never spins.
      inertia: Infinity,
      plugin: {
